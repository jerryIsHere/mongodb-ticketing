...
export interface EventModel extends Model<IEvent, EventQueryHelpers, IEventMethod> {
}
...
export const eventSchema =
    new Schema<IEvent, EventModel, IEventMethod, EventQueryHelpers>({
        ...
        saleInfos: {
            type: [saleInfoSchema], required: true,
            validate: {
...
eventSchema.path('venueId').validate(async function (val) {
    ...
    if (tickerFromOtherVenue != null)
        throw new Error(`Update of ${singular_name} with id ${eventId} failed ` +

...
import { collection_name as venue_collection_name } from "./venue";
interface IEvent {
    eventname: string;
    datetime: Date;
...
const eventSchema = new Schema<IEvent>({
    ...
    duration: { type: Number, required: true, min: [0, 'Duration must be greater then 0.'] },
    ...
    venueId: { type: Schema.Types.ObjectId, ref: venue_collection_name ,required: true },
})
const collection_name = "events"
referentialIntegrity = function(event: IEvent){
...
export const eventModel = model<IEvent>(collection_name, eventSchema)
